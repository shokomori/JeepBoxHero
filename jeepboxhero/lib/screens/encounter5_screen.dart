...
class _Encounter5ScreenState extends State<Encounter5Screen> {
  ...
  final List<Map<String, dynamic>> _finalDialogues = [
    ...
    {
      'type': 'dialogue',
      'text': 'A voice that carried us all. Some things truly never fade.',
      'speaker': 'Aling Fe',
    },
  ];
  ...
  void initState() {
    ...
    try {
      AudioManager()
          .playBgm('bgm_aling_fe_theme.mp3', volume: 0.18, loop: true);
    } catch (_) {}
  ...
  void _startGameplay() async {
    final result = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => ShelvesScreen(
          targetAlbumTitle: 'Lea Salonga',
          targetAlbumArtist: 'Lea Salonga',
          successNarration:
    ...
    if (result == true && mounted) {
      GameState.addToCart({
        'album': 'Lea Salonga',
        'artist': 'Lea Salonga',
        'imagePath': 'assets/albums/lea_salonga.png',
      });
  ...
  bool _isCustomerSpeaking() {
    ...
    return speaker != null && speaker.toString().contains('Aling Fe');
  }
        ...
        child: Stack(
          children: [
            ...
            AnimatedPositioned(
              ...
              child: Image.asset(
                _isCustomerSpeaking()
                    ? 'assets/characters/aling_fe_talking.png'
                    : 'assets/characters/aling_fe.png',
                fit: BoxFit.contain,
                errorBuilder: (context, error, stackTrace) {
  ...
  Widget _buildDialogueOptions(double w, double h) {
    final options = [
      {'text': 'Sounds like a national treasure.', 'value': 'A'},
      {'text': 'Let me bring her voice back to you.', 'value': 'B'},
      {'text': 'She carried us all with every note.', 'value': 'C'},
    ];
