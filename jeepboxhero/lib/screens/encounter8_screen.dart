// lib/screens/encounter8_screen.dart
...
class _Encounter8ScreenState extends State<Encounter8Screen> {
  ...
  final List<Map<String, dynamic>> _dialogues = [
    {
      'type': 'narration',
      'text':
          'A young woman with tearful eyes enters, clutching her arms to her chest.',
      'speaker': null,
    },
    {
      'type': 'dialogue',
  ...
  final List<Map<String, dynamic>> _postChoiceDialogues = [
    {
      'type': 'narration',
      'text':
          '• Check the OPM/Acoustic section.\n• Look for a gentle cover—Moira facing forward, hair flowing softly around her.\n• It’s calm and sincere, like a quiet strength wrapped in melancholy.','speaker': null,
    },
  ];
  ...
  final List<Map<String, dynamic>> _finalDialogues = [
    {
      'type': 'narration',
      'text': 'She exits, a little lighter.',
      'speaker': null,
    },
  ];
  ...
  void initState() {
    ...
    _updateShowContinue();
    try {
      AudioManager().playBgm('bgm_mara_theme.mp3', volume: 0.18, loop: true);
    } catch (_) {}
    try {
  ...
  void _startGameplay() async {
    final result = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => ShelvesScreen(
          targetAlbumTitle: 'Malaya',
          targetAlbumArtist: 'Moira Dela Torre',
          successNarration:
              'The player flips through muted covers until finding one that radiates quiet sorrow.',
          successDialogue:
    ...
    if (result == true && mounted) {
      GameState.addToCart({
        ...
        'imagePath': 'assets/albums/moira_malaya.png',
      });
            ...
            onTap: _albumFound
                ? () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => VinylTableScreen(encounterNumber: 8),
                      ),
                    );
                  }
                : null,
                  ...
                  onTap: _albumFound
                      ? () async {
                          final result = await Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => ReceiptTableScreen(encounterNumber: 8),
                            ),
                          );
 ...
 if (_showContinue && !(_dialogueIndex >= _dialogues.length - 1))
  ...
  Widget _buildDialogueOptions(double w, double h) {
    final options = [
      {'text': 'Heartbreak never sounded sweeter.', 'value': 'A'},
      {'text': 'I\'ll get you through this.', 'value': 'B'},
      {'text': 'Sometimes sad songs heal best.', 'value': 'C'},
    ];
