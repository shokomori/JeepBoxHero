// lib/screens/encounter4_screen.dart
...
class _Encounter4ScreenState extends State<Encounter4Screen> {
  ...
  final List<Map<String, dynamic>> _dialogues = [
    {
      'type': 'narration',
      'text':
          'A young man in a neon jacket leans on the counter, eyes sharp with curiosity.',
      'speaker': null,
    },
    {
      'type': 'dialogue',
  ...
  final List<Map<String, dynamic>> _finalDialogues = [
    ...
    {
      'type': 'dialogue',
      'text': 'Real stories never get old. Keep fighting the good fight.',
      'speaker': 'DJ Quenta',
    },
  ];
  ...
  void initState() {
    ...
    try {
      AudioManager()
          .playBgm('bgm_dj_quenta_theme.mp3', volume: 0.18, loop: true);
    } catch (_) {}
  ...
  void _startGameplay() async {
    final result = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => ShelvesScreen(
          targetAlbumTitle: 'Mga Kwento ng Makata',
          targetAlbumArtist: 'Gloc-9',
          successNarration:
    ...
    if (result == true && mounted) {
      GameState.addToCart({
        ...
        'artist': 'Gloc-9',
        'imagePath': 'assets/albums/gloc9_kwento.png',
      });
  ...
  void _exitCustomer() {
    ...
    Future.delayed(const Duration(milliseconds: 600), () async {
      ...
      // Navigate to Encounter5Screen when ready
      if (mounted) {
        Navigator.of(context).pushReplacement(PageRouteBuilder(
          transitionDuration: const Duration(milliseconds: 500),
          pageBuilder: (context, animation, secondaryAnimation) =>
              const Encounter5Screen(),
  ...
  bool _isCustomerSpeaking() {
    ...
    return speaker != null && speaker.toString().contains('DJ Quenta');
  }
        ...
        child: Stack(
          children: [
            ...
            AnimatedPositioned(
              ...
              child: Image.asset(
                _isCustomerSpeaking()
                    ? 'assets/characters/dj_quenta_talking.png'
                    : 'assets/characters/dj_quenta.png',
                fit: BoxFit.contain,
                errorBuilder: (context, error, stackTrace) {
            ...
            if (_albumFound)
              Positioned(
                ...
                height: w * 0.25,
                child: Image.asset(
                  'assets/albums/gloc9_kwento.png',
                  fit: BoxFit.contain,
                  errorBuilder: (context, error, stackTrace) {
                    return Container(
                  ...
                  onTap: _albumFound
                      ? () async {
                          final result = await Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => ReceiptTableScreen(encounterNumber: 4),
                            ),
                          );
                ...
                onTap: _albumFound
                    ? () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => VinylTableScreen(encounterNumber: 4),
                          ),
                        );
                      }
                    : null,
 ...
 if (_showContinue && !(_dialogueIndex >= _dialogues.length - 1))
  ...
  Widget _buildDialogueOptions(double w, double h) {
    final options = [
      {'text': 'Lyrics sharper than knives, huh?', 'value': 'A'},
      {'text': 'I\'ll find those street stories.', 'value': 'B'},
      {'text': 'Truth always rhymes better.', 'value': 'C'},
    ];
